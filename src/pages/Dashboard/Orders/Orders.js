...
const Orders = () => {
  ...
  useEffect(() => {
    fetch("https://hidden-sea-94069.herokuapp.com/allorders")
      .then((res) => res.json())
  ...
  const handleUpdate = (id) => {
    ...
    if (proceed) {
      setStatus(status);
      fetch(`https://hidden-sea-94069.herokuapp.com/updateStatus/${id}`, {
        method: "PUT",
  ...
  const deleteHandler = (id) => {
    ...
    if (proceed) {
      fetch(`https://hidden-sea-94069.herokuapp.com/allorders/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
