...
const YourOrders = () => {
  ...
  useEffect(() => {
    fetch(
      `https://hidden-sea-94069.herokuapp.com/myorders?email=${user.email}`
    )
      .then((res) => res.json())
  ...
  const deleteHandler = (id) => {
    ...
    if (proceed) {
      fetch(`https://hidden-sea-94069.herokuapp.com/myorders/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    <div>
      <Container>
        <h1>My  <strong className="text-danger">Order's</strong></h1>
